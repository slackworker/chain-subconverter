...
func (handler *Handler) handleRuntimeStatus(writer http.ResponseWriter, request *http.Request) {
	if handler.runtimeStatus == nil {
		writeBlockingError(writer, request, http.StatusServiceUnavailable, "INTERNAL_ERROR", "runtime status unavailable", "global", nil, nil)
		return
	}
	...
	if err != nil {
		writeBlockingError(writer, request, http.StatusInternalServerError, "INTERNAL_ERROR", "runtime status unavailable", "global", nil, nil)
		return
	}
