...
func TestLoadServerFromEnv(t *testing.T) {
	tests := []struct {
		...
		{
			name: "invalid read requests per minute",
			env: map[string]string{
				EnvReadRequestsPerMinute: "bad",
			},
			wantErr: "parse CHAIN_SUBCONVERTER_READ_REQUESTS_PER_MINUTE",
		},
		{
...
func TestServerValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "negative read requests per minute",
			cfg: Server{
				HTTPAddress:                 DefaultHTTPAddress,
				UserFacingBaseURL:           DefaultUserFacingBaseURL,
				SubconverterFacingBaseURL:   DefaultSubconverterFacingBaseURL,
			...
			wantErr: "read requests per minute must not be negative",
		},
		{
