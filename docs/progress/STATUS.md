...
- 术语边界统一以 [spec/01-overview](../spec/01-overview.md) 为准；本文只记录进度与缺口，不重复定义“模板 URL / 模板内容 / `baseCompleteConfig` / `completeConfig`”。

- `Phase 1` 已完成：`subconverter` 真实 `3-pass` 集成已落地
- `Phase 2` 已完成（最小范围）：固定测试数据与默认值下，服务层能产出 `stage2Init`、编码 `longUrl`、渲染最终订阅 YAML，且已通过最小 HTTP 层对外暴露上述闭环（见下文「已完成」）
...
| 层 | 职责 | 不包含 |
|----|------|--------|
| **internal/service** | 业务规则：`stage2Init`、长链接逻辑载荷编解码、最终订阅 YAML 渲染、3-pass 结果解析为 `ConversionFixtures`、与 spec 对齐的校验与错误类型（对调用方以错误值表达） | HTTP 状态码、路由、JSON 响应形状；不直接暴露 subconverter URL 拼接细节以外的集成策略（集成集中在 `internal/subconverter`） |
...
| **internal/review/testdata/3pass-ss2022-test-subscription/** | `internal/review` 的稳定自动化 fixture：输入、3-pass 输出、阶段 2 snapshot 与最终订阅 YAML golden | 手动 Front-end Review 工作区 |
| **review/cases/3pass-ss2022-test-subscription/** | 手动 Front-end Review 工作区：可编辑输入、可选本地 HTTP fixture、Stage1/Stage2 运行产物 | 自动化测试真相源 |
...
- `internal/api` 仅有最小端点；`POST /api/resolve-url`、`POST /api/short-links`、短链语义、`GET /subscription/<id>.yaml` 等仍属 Phase 3
