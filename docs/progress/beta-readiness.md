...
- **阶段**：3.0 Beta 线；**`v3.0.0-beta.1`**（2026-05-24）、**`v3.0.0-beta.2`**（2026-05-25）已发布；滚动标签 **`beta-latest`** 与 beta.2 同期 digest。
...
- **`beta-latest` 实战**（**2026-05-25**，beta.2）：vps-01 / vps-02 内网与公网一体化、`beta-latest` + `deployed-smoke` **通过**（digest `sha256:afa71279…`；见 [third-party-deployments.md](../testing/third-party-deployments.md)）。
- **`beta-latest` 实战**（**2026-05-23**，beta.1）：同上形态 **通过**；Railway + Koyeb 双 Docker 分离形态 **通过**。

## 距下一版 Beta（beta.N）仍缺

1. **反馈闭环**：`.github/ISSUE_TEMPLATE/` 已补；持续用 Issue/回归记录归档。
2. **E2E 加深（非硬阻塞）**：阻断路径、更广 scheme 矩阵；blocking 仍为两条 mocked happy path。
3. **质量债**：`subconverter` 浮动 tag 需在回归中注明；B/C workflow log 视觉与 default 未完全统一；plan 文件待 beta 线稳定后并入 STATUS。
...
2. `dev` 合并到 `beta`，打 tag，等 GHCR `beta-latest` 就绪。
3. 抽样第三方设备 smoke + 更新 digest / 回归记录；视需要刷新在线预览（Koyeb）。
4. 补 E2E 阻断路径；plan 并入 STATUS 并删除 plan 文件。
