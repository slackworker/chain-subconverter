...
本文档说明 chain-subconverter 的 SSRF（Server-Side Request Forgery，服务端请求伪造）防护策略、配置方式及使用风险。**基于旧版本编写，策略仍可参考；当前实现以代码与 [legacy 目录规范说明](../legacy/README.md) 为准。**
...
- **C**：远程订阅源 URL（用户提供的 `source.remote_url`）

### 1.2 SSRF 攻击原理

攻击者（如恶意订阅者 A）可构造异常的 `source.remote_url`，诱使 **B 这台服务器** 向不该访问的目标发起请求，例如：
...
| 服务 | 默认端口 | 说明 |
|------|----------|------|
| [subconverter](https://github.com/slackworker/subconverter/tree/integration/chain-subconverter) | 25500 | 订阅转换服务 |
| [SubStore](https://hub.docker.com/r/xream/sub-store) | 3001 | 订阅管理后端 API |
