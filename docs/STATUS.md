...
| `dev` | `web/src/scheme/a|b|c` 方案演进与对照 | `dev-latest`（手动） |
...
同一分支可同时预览 `/ui/a`、`/ui/b`、`/ui/c`；多 worktree 并行须设 `CHAIN_SUBCONVERTER_DEV_UP_PORT_OFFSET`（见 [local-dev-smoke.md](testing/local-dev-smoke.md)）。
