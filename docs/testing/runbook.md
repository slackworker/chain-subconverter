...
发版 / beta.N 前**检查清单**。分支与镜像口径见 [../STATUS.md](../STATUS.md)；部署见 [deploy/README.md](../../deploy/README.md)；设备结论**覆盖**写入 [deployments.md](deployments.md)（不滚历史）。
...
记录字段见 [deployments.md](deployments.md)。
...
发版后：**覆盖** [deployments.md](deployments.md) 已测形态节与覆盖表；[STATUS.md](../STATUS.md) 更新页眉与 §最近验证「第三方部署」一行（digest 不重复抄写，见 [MAINTENANCE.md](../MAINTENANCE.md)）。
