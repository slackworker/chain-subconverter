...
- `web/src/scheme/default/*`
- `web/src/scheme/plain/*`
- 后续 `phase4-ui-a` / `phase4-ui-b` / `phase4-ui-c` 方案分支中的具体 UI 实现
...
- `build` 通过，表示当前默认方案仍能消费共享业务层
- `build:plain` 通过，表示第二套极简占位方案也能消费同一共享业务层
- 两者都通过时，才能说明“共享入口不再直接依赖默认方案组件”已具备可执行证据
