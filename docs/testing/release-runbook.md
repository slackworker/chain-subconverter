...
- [ ] 自动化：`go test`、web 单测、E2E（`default-happy-path`、`port-forward-happy-path`）、`build:default`（探索性 `b1`/`b2`/`c1`/`c2` 非发布门禁）、`docker compose config` — 见 [local-dev-smoke.md](local-dev-smoke.md)「发布前完整检查」与 [test-system-review.md](test-system-review.md) CI 门禁
