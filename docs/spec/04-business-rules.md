...
- **上游 `User-Agent`（拉机场用）**：本项目调用 `subconverter` 时必须设置 HTTP `User-Agent` 请求头（不依赖上游 `ua=` 查询参数）。默认值为 `clash.meta/1.19.20`。`GET /sub` 与 `GET /sub/<id>` 若收到非空且**非浏览器类**的客户端 `User-Agent`（Clash / Mihomo / clash-verge 等订阅客户端），则将该值作为上游 `User-Agent`；浏览器类 `User-Agent`（大小写不敏感，命中任一子串即判定：`Mozilla/`、`Chrome/`、`Chromium/`、`Safari/`、`Firefox/`、`Edg/`、`Edge/`、`OPR/`、`Opera/`、`MSIE `、`Trident/`）以及空/空白 `User-Agent` 不转发，一律使用默认值。`convert` / `generate` / `resolve-url` 等浏览器侧入口不得转发客户端 UA，一律使用默认值。上游 `subconverter` 拉远程订阅时通常会转发该请求头
