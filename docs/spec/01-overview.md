...
- **单一生成路径**：统一先调用 `subconverter` 生成完整配置，再执行阶段 2 改写
...
| 阶段 3：输出区 | 基于阶段 1 与阶段 2 快照生成最终 YAML、长链接，以及可选短链接 |
...
| 完整配置（CompleteConfig） | `subconverter` 生成的完整 Mihomo YAML，是阶段 2 改写的唯一基础 |
