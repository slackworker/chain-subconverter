...
帮助用户基于其**已有信息**（订阅/YAML/节点/中转机 `server:port` 等），通过 **Web 前端**集中完成 **Mihomo** 的**链式代理**和/或**端口转发**配置生成与输出：系统自动解析输入、自动检测中转信息是否包含**通用配置**（见下文术语），必要时通过 **转换模板 + subconverter** 生成**完整配置**，随后按用户指定方式完成改写并输出可直接供 Mihomo 使用的最终 YAML（可选提供订阅链接或 YAML 文件下载），避免用户手动编辑 YAML 或进行任何代码操作。范围上仅针对 Mihomo 内核，暂不涉及 Surge、 Loon 等其他格式（预留兼容）。
