...
- **中转信息输入**（可能是完整配置 / 订阅 / 节点-only / 端口转发中转机 `server:port`）
- **落地信息输入**（可能是完整配置 / 订阅 / 节点-only）

系统应先解析可用的输入源并判定“是否包含完整配置”，然后按以下规则确定**基准（完整）Mihomo 配置**：

| 中转输入是否存在/含完整配置 | 落地输入是否存在/含完整配置 | 基准配置来源 | 说明 |
|---|---|---|---|
| 只提供一份输入，且不含完整配置 | （另一份不存在） | **内置订阅转换生成** | 将这份输入解析得到的**全部节点**作为节点集合，使用固定模板生成完整配置骨架，再进入后续“拼接/修改”。 |
| 只提供一份输入，且含完整配置 | （另一份不存在） | **使用该输入的完整配置** | 在同一份配置中识别/选择中转与落地节点，无需再做“跨输入源合并”。 |
| 否 | 否 | **内置订阅转换生成** | 将两份输入解析得到的**全部节点**合并为节点集合，使用固定模板生成完整配置骨架，再进入后续“拼接/修改”。 |
| 是 | 否 | **使用中转输入的完整配置** | 将落地节点（若不在基准中）拼接进 `proxies`，再进入后续“修改”。 |
| 否 | 是 | **使用落地输入的完整配置** | 将中转节点（若需要且不在基准中）拼接进 `proxies`，再进入后续“修改”。 |
| 是 | 是 | **必须由用户选择** | 前端必须提示用户“请选择以哪一份为基准配置”；API 必须要求显式字段 `base_config_preference: "transit" | "landing"`，缺失则返回可读错误。 |
