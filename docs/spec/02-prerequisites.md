...
| **3. 落地/出口信息（Landing / Egress）** | 必填 | 用户最终出口节点集合。可来自基准配置、也可由用户单独提供并被拼接进入输出；必须由用户指定/确认“哪些节点是落地节点”。 |
...
### 1.5 两份输入源同时存在时：基准配置选择规则（前端必须提示，API 必须显式传入）

Web App 场景下，用户会分别提供两份信息：

- **中转信息输入**（可能是完整配置 / 订阅 / 节点-only / 端口转发中转机 `server:port`）
- **落地信息输入**（可能是完整配置 / 订阅 / 节点-only）

系统应先解析两份输入并判定“是否包含完整配置”，然后按以下规则确定**基准（完整）Mihomo 配置**：

| 中转输入是否含完整配置 | 落地输入是否含完整配置 | 基准配置来源 | 说明 |
|---|---|---|---|
