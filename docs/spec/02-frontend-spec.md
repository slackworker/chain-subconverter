...
- 阶段 3 必须提供以下能力：当前链接展示与编辑、短链切换、打开预览、复制、下载、反向解析、消息展示
...
- 阶段 3 必须提供单一的当前链接输入框
- 该输入框同时承担链接展示与用户手动编辑输入，不区分“输入/输出”两套控件或状态切换
...
- 方案层可在不改变共享语义的前提下，对当前链接输入框补充局部提示，不改变后端 `scope` 契约
