# 02 - 前端 UI 规格

> 本章只定义界面结构、展示语义与交互边界。自动识别和配置改写规则见 [04-business-rules](04-business-rules.md)，接口字段见 [03-backend-api](03-backend-api.md)。

---

## 全局布局

### Nav Bar

| 元素 | 说明 |
|------|------|
| Title | `Chain Converter for Mihomo` |
| 亮/暗色主题切换 | 切换 light / dark mode |
| GitHub 跳转 | 图标链接至项目 GitHub 仓库 |

> 当前版本仅设计中文界面，不提供语言切换。

### 响应式设计

- 阶段 1 到阶段 3 均采用纵向单列堆叠布局
- 桌面端通过更宽容器与更舒展间距提升可读性，不改为双列
- 移动端保持相同顺序，自上而下滚动操作

---

## 阶段 1：输入区

阶段 1 负责收集原始输入，并触发“转换并自动识别”。

### 1.1 落地节点信息输入区

- 位置：输入区最上方
- 用途：输入落地节点或落地订阅信息
- 输入方式：订阅 URL、节点 URI、手动添加 SOCKS5 节点
- 展示规则：每条一行，显示行号，不自动换行，允许横向滚动

#### 1.1.1 手动添加 SOCKS5 节点

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | text | 是 | 节点名称 |
| `server` | text | 是 | 服务器地址 |
| `port` | number | 是 | 端口号 |
| `username` | text | 否 | 用户名 |
| `password` | text | 否 | 密码 |

- 仅支持手动添加 SOCKS5
- `username` 与 `password` 必须成对出现
- 提交后以一条标准 `socks5://` URI 追加到落地输入区

#### 1.1.2 落地副本

- 同一落地节点的多个副本只能在阶段 1 创建
- 多条完全一致的 URI 不去重，视为用户显式创建副本
- 副本名称由后端稳定重命名，供阶段 2 第一列展示

### 1.2 中转信息输入区

- 位置：落地输入区下方
- 用途：输入中转节点或中转订阅信息
- 输入方式：订阅 URL 或节点 URI
- 展示规则：每条一行，显示行号，不自动换行，允许横向滚动

### 1.3 端口转发服务信息区

- 启用前提：先展开高级菜单，再开启“启用端口转发（实验性）”
- 默认状态：隐藏
- 布局：位于中转信息区下方，高级菜单区上方
- 输入格式：`server:port`
- 输入规则：支持多行；每行一条；保留输入顺序

### 1.4 高级菜单区

- 位置：位于中转信息区下方；若端口转发输入区显示，则位于其下方
- 形态：默认折叠

| 选项 | 类型 | 说明 |
|------|------|------|
| 客户端 | 下拉菜单 | 当前仅 Clash/Mihomo |
| 转换模板 | 只读展示或单选项下拉 | 当前仅默认模板 |
| Emoji | checkbox | 是否在节点名前加 emoji |
| 启用 UDP | checkbox | 是否启用 UDP |
| 跳过证书验证 | checkbox | 是否跳过 SSL 证书验证 |
| 启用端口转发（实验性） | switch | 开启后显示端口转发服务输入区 |

### 1.5 转换按钮

- 按钮文案：`转换并自动识别`
- 功能：调用 `POST /api/stage1/convert`
- 成功后：使用后端返回的 `stage2Init` 直接初始化阶段 2

### 1.6 阶段 1 交互约束

- 布局顺序固定为：落地信息 -> 中转信息 -> 条件显示的端口转发服务信息 -> 高级菜单 -> 转换按钮
- 修改阶段 1 任一输入后，必须重新执行“转换并自动识别”
- 阶段 1 不负责直接生成最终 YAML

---

## 阶段 2：配置区

阶段 2 以“每个落地节点一行”的固定模型渲染，数据完全来自后端返回的 `stage2Init`。

### 2.1 行模型

| 字段 | 类型 | 说明 |
|------|------|------|
| `rowId` | string | 前端稳定标识 |
| `landingNodeName` | string | 本行对应的落地节点名称 |
| `allowedModes` | `mode[]` | 本行第二列允许展示的模式列表 |
| `mode` | `none \| chain \| port_forward` | 当前选择的配置方式 |
| `targetName` | `string \| null` | 第三列当前值 |

### 2.2 整体布局

| 列 | 内容 | 交互 |
|----|------|------|
| 第一列：落地节点 | `landingNodeName` | 只读 |
| 第二列：配置方式 | `mode` | 仅展示 `allowedModes` 中的选项 |
| 第三列：目标 | `targetName` | 由 `mode` 决定数据源与控件状态 |

### 2.3 第一列：落地节点

- 数据来源：`stage2Init.rows[]`
- 展示语义：只展示，不允许替换或新增

### 2.4 第二列：配置方式

- `none`：不修改该落地节点
- `chain`：第三列从 `stage2Init.chainTargets[]` 中选择
- `port_forward`：第三列从 `stage2Init.forwardRelays[]` 中选择
- 若某模式不在 `allowedModes` 中，则前端不得展示该选项

### 2.5 第三列：目标

- 当 `mode = none` 时，第三列清空并禁用
- 当 `mode = chain` 时，第三列展示链式候选列表
- 当 `mode = port_forward` 时，第三列展示端口转发服务列表
- 前端不自行推导候选列表，不自行重做自动识别；只消费后端产物

### 2.6 配置按钮

- 按钮文案：`配置`
- 功能：提交阶段 1 输入快照、阶段 2 配置快照和短链接开关，调用 `POST /api/generate`
- 成功后：进入阶段 3

---

## 阶段 3：输出区

### 3.1 链接展示区

- 长链接：规范化快照链接
- 短链接：长链接的后端别名
- 前端可提供“使用短链接”开关，但长链接始终为规范化来源

### 3.2 操作按钮

| 按钮 | 功能 |
|------|------|
| 打开 | 在新标签页打开 YAML 内容 |
| 复制 | 复制 YAML 内容到剪贴板 |
| 下载 | 下载 `.yaml` 文件 |

### 3.3 消息区

- 展示后端返回的 `messages[]` 与 `blockingErrors[]`
- 前端统一按 `level` 渲染普通消息
- 阻断错误应显著展示，并阻止进入成功态

---

## 跨阶段交互

- 三个阶段依次展开，或用 stepper/tab 导航
- 重新执行阶段 1 后，阶段 2 必须完全按新的 `stage2Init` 重建
- 阶段 3 的长链接可作为页面状态恢复来源

---

## 前端边界

- 前端不负责解析 `completeConfig`
- 前端不负责从节点名推导区域、从 `proxy-groups` 过滤候选、或决定默认模式
- 前端只负责展示后端返回的初始化结果、允许用户手动修改，并提交最终快照
