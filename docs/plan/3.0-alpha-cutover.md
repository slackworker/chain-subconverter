...
- 安全侧已补上最小 SSRF 拒绝名单：[internal/service/managed_conversion_source.go](../../internal/service/managed_conversion_source.go) 现已默认拒绝 loopback / link-local / RFC1918 / ULA / multicast / unspecified 模板目标，并保留 `CHAIN_SUBCONVERTER_TEMPLATE_ALLOW_PRIVATE_NETWORKS=true` 作为可信自部署环境的显式放行开关；[internal/config/server.go](../../internal/config/server.go) 也已支持 `CHAIN_SUBCONVERTER_REQUIRE_PUBLIC_BASE_URL=true`，可在未显式设置 `PUBLIC_BASE_URL` 时阻止服务启动；[internal/api/rate_limit.go](../../internal/api/rate_limit.go) 已为四个写接口补上最小 per-IP 限速。剩余安全缺口主要是更严格的出站控制与域名白名单。
- 文档：`docs/temp/completed-phases/phase-4-*` 仍保留选型期叙述；[../progress/STATUS.md](../progress/STATUS.md) 仍混有“A/B/C 评审尚未完成”“后续看旧 plan”等旧口径；`docs/temp/*` 与 `_legacy/` 仍需继续收口。
...
- 删除 / 归档已完成 plan：[phase-4-breakdown.md](../temp/completed-phases/phase-4-breakdown.md)、[phase-4-dev-readiness.md](../temp/completed-phases/phase-4-dev-readiness.md) 全文移入 `docs/temp/completed-phases/` 后从主导航移除（与现行 docs/temp 规则一致），ROADMAP 标注当前执行入口为本计划。
...
- W1.1 [已完成] 把 [phase-4-breakdown.md](../temp/completed-phases/phase-4-breakdown.md) 与 [phase-4-dev-readiness.md](../temp/completed-phases/phase-4-dev-readiness.md) 移入 `docs/temp/completed-phases/`；并从 [../README.md](../README.md) 主导航移除。
...
- W3.3 [已完成] 给 `/api/stage1/convert`、`/api/generate`、`/api/short-links`、`/api/resolve-url` 加最简 per-IP 限速（默认 60/min，可由 env 关闭）。
