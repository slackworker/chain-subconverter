...
import {
	...
	getWorkflowLogLevelLabel,
	getWorkflowSourceLabel,
	getWorkflowStageLabel,
} from "../../lib/workflow-log-display";
import { AlertCircleIcon, AlertTriangleIcon, ChevronDownIcon, InfoIcon, TerminalIcon } from "./Icons";
				...
				<div className="absolute bottom-full right-0 mb-4 w-80 max-h-[480px] bg-zinc-950/95 backdrop-blur-2xl border border-zinc-800/80 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out">
					<div className="p-4 border-b border-zinc-800/50 flex items-center justify-between bg-zinc-900/30">
						<div className="flex items-center gap-2.5">
							...
							<h3 className="text-sm font-semibold text-zinc-100">工作流日志</h3>
					...
					<div className="overflow-y-auto p-2.5 space-y-1.5 custom-scrollbar bg-zinc-950/50">
						{entries.slice().reverse().map((entry) => (
							entry.kind === "separator" ? (
								<div key={entry.id} className="px-2 py-2 text-[11px] font-semibold tracking-wide text-zinc-500 border-y border-dashed border-zinc-800/80">
									{entry.message}
								</div>
							) : (
								<div key={entry.id} className="p-2.5 rounded-xl hover:bg-white/[0.03] transition-colors group/item">
										...
										<div className="flex-1 min-w-0">
											...
											<div className="flex flex-wrap items-center gap-2 mt-1.5">
												...
												<span className="text-[10px] text-zinc-600 bg-zinc-900 px-1.5 py-0.5 rounded">
													{getWorkflowSourceLabel(entry.source)}
												</span>
												<span className={`text-[9px] px-1 py-0.5 rounded font-bold uppercase tracking-wider ${getLevelColor(entry.level)}`}>
												...
												{entry.originStage && (
													<span className="text-[10px] text-zinc-600 bg-zinc-900 px-1.5 py-0.5 rounded uppercase tracking-widest font-bold">
														{getWorkflowStageLabel(entry.originStage, "zh", "short")}
													</span>
												)}
											</div>
										</div>
