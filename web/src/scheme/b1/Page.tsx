...
export function SchemePage({ workflow, outputActions, runtimeConfig }: AppPageProps) {
	...
	return (
		<div className={`min-h-screen transition-colors duration-300 font-sans ${colorMode === "dark" ? "dark bg-[#0a0a0c] text-zinc-300" : "bg-slate-50 text-slate-700"}`}>
			{/* Header */}
			<header className={`border-b backdrop-blur-md sticky top-0 z-40 transition-colors duration-300 ${colorMode === "dark" ? "border-zinc-800/80 bg-zinc-950/50 text-zinc-100" : "border-slate-200 bg-white/70 text-slate-800"}`}>
				<div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
					<div className="flex items-center gap-3">
						<img
							...
							aria-hidden="true"
						/>
						<h1 className="text-lg font-bold tracking-tight">{locale === "zh" ? "链式代理 · 订阅转换" : "Chain Subconverter"}</h1>
					</div>
			...
			<main className="max-w-5xl mx-auto px-6 py-10 flex flex-col gap-8">
				<div className="flex flex-col gap-2 mb-2">
					...
					<p className={colorMode === "dark" ? "text-zinc-400" : "text-slate-500"}>
						{copy.stage1Desc}
					</p>
				</div>

				<Stage1 workflow={workflow} locale={locale} colorMode={colorMode} runtimeConfig={runtimeConfig} />
				<Stage2 workflow={workflow} locale={locale} colorMode={colorMode} />
				<Stage3 workflow={workflow} outputActions={outputActions} locale={locale} colorMode={colorMode} />
			</main>

			<footer className={`border-t mt-12 py-8 text-center text-sm transition-colors duration-300 ${colorMode === "dark" ? "border-zinc-800/80 text-zinc-500" : "border-slate-200 text-slate-400"}`}>
				<p>Chain Subconverter &copy; {new Date().getFullYear()} - Designed with Scheme B</p>
			</footer>

			{/* Log Area */}
			<LogPanel entries={workflow.workflowLog} locale={locale} />
		</div>
	);
}
