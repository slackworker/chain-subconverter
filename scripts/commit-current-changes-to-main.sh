...
TEMP_DIR=
MAIN_WORKTREE=
MAIN_WORKTREE_CREATED=0
MAIN_PATCH_APPLIED=0
ORIGINAL_BRANCH=
STAGED_STASHED=0
UNSTAGED_STASHED=0
...
require_clean_git_state_at() {
  local path=$1
  local label=$2

  [[ ! -d "$(git -C "$path" rev-parse --git-path rebase-merge)" ]] || fail "$label rebase is already in progress"
  [[ ! -d "$(git -C "$path" rev-parse --git-path rebase-apply)" ]] || fail "$label rebase or apply is already in progress"
  [[ ! -f "$(git -C "$path" rev-parse --git-path MERGE_HEAD)" ]] || fail "$label merge is already in progress"
  [[ ! -f "$(git -C "$path" rev-parse --git-path CHERRY_PICK_HEAD)" ]] || fail "$label cherry-pick is already in progress"
}
...
cleanup() {
  ...
  if [[ "$status" -ne 0 ]]; then
    if [[ "$MAIN_PATCH_APPLIED" -eq 1 && "$MAIN_WORKTREE_CREATED" -eq 0 && -n "$MAIN_WORKTREE" ]]; then
      log "clearing failed apply from existing '$MAIN_BRANCH' worktree"
      git -C "$MAIN_WORKTREE" reset --hard HEAD >/dev/null 2>&1 || true
    fi
  ...
  if [[ "$MAIN_WORKTREE_CREATED" -eq 1 && -n "$MAIN_WORKTREE" && -d "$MAIN_WORKTREE" ]]; then
...
if MAIN_WORKTREE=$(branch_worktree_path "$MAIN_BRANCH"); then
  log "using existing '$MAIN_BRANCH' worktree: $MAIN_WORKTREE"
else
  MAIN_WORKTREE="$TEMP_DIR/$MAIN_BRANCH-worktree"
  log "creating temporary '$MAIN_BRANCH' worktree: $MAIN_WORKTREE"
  git -C "$ROOT_DIR" worktree add "$MAIN_WORKTREE" "$MAIN_BRANCH" >/dev/null
  MAIN_WORKTREE_CREATED=1
fi

require_clean_worktree "$MAIN_WORKTREE" "'$MAIN_BRANCH'"
...
MAIN_PATCH_APPLIED=0
if [[ "$MAIN_WORKTREE_CREATED" -eq 1 ]]; then
